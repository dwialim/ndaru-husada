    ...
    <div class="col-md-12">
        <div class="card ">
            <div class="card-header bg-{{ (!empty($data)) ? 'primary' : 'success' }}">
                @empty($data)
                    <h3 class="text-white">{{ __('Tambah Data '.$jenis.' Baru') }}</h3>
                    @else
                    <h3 class="text-white">{{ __('Edit Data '.$jenis.' Baru') }}</h3>
                @endempty
            </div>
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label for="nama">{{ __('Nama '.$jenis.'') }}</label>
                                <input id="nama" type="text" class="form-control"
                                    name="nama" value="{{ (!empty($data)) ? $data->nama : '' }}" placeholder="Nama {{ $jenis }}" required>
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label for="satuan">{{ __('Satuan '.$jenis.'') }}</label>
                                {{-- <input id="Satuan {{ $jenis }}" type="text" class="form-control" name="satuan" value=""
                                    placeholder="Satuan {{ $jenis }}" required> --}}
                                <select class="form-control select2" id="satuan" name="satuan">
                                    <option value="">{{ __('--Pilih Satuan--') }}</option>
                                    @foreach ($satuan as $p)
                                        @empty(!$data)
                                            @if (($data->satuan->id ?? 0) == ($p->id))
