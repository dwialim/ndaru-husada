...
class UserController extends Controller
{
        ...
        return Datatables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    if (Auth::user()->can('manage_user')) {
                        ...
                        return '<div class="table-actions">
                                <a href="'.url('user/'.$data->id).'" ><i class="ik ik-edit-2 f-16 mr-15 text-green"></i></a>
                                <a href="#" onclick="delete_user('.$data->id.')"><i class="ik ik-trash-2 f-16 text-red"></i></a>
                            </div>';
                    } else {
                        return '';
                    }
    ...
    public function edit($id)
    {
        try {
            ...
            if ($user) {
                ...
                return view('master.users.user-edit', compact('user', 'user_role', 'roles'));
            } else {
                return redirect('404');
            }
        } catch (\Exception $e) {
    ...
    public function delete(Request $request)
    {
        ...
        if ($user) {
            $user->delete();
            return ['status' => 'success', 'message' => 'Anda Berhasil Menghapus User', 'title' => 'Success'];
        } else {
            return ['status' => 'error', 'message' => 'User Gagal Dihapus', 'title' => 'Whoops'];
        }
    }
}
