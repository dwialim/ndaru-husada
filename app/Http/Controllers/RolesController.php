...
class RolesController extends Controller
{
    ...
    {
        try {
            $permissions = Permission::pluck('nama_level', 'id');
        ...
        return Datatables::of($data)
                ->addColumn('permissions', function ($data) {
                    ...
                    foreach ($roles as $key => $role) {
                        $badges .= '<span class="badge badge-dark m-1">'.$role->nama_level.'</span>';
                    }
                    if ($data->nama_level == 'Super Admin') {
    ...
    public function create(RoleRequest $request)
    {
        ...
        try {
            $role = Role::create(['nama_level' => $request->role]);
